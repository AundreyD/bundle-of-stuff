...
month (after the payment for that month is made). The monthly payment must be a multiple of $10 
and is the same for all months. Notice that it is possible for the balance to become negative using 
...
def postBalance(balance, annualInterestRate, fixed_monthly = 10):
  remaining_balance = balance
  month = 1
  monthlyInterestRate = annualInterestRate / 12.0
  while month <= 12:
    remaining_balance -= fixed_monthly
    remaining_balance = remaining_balance + (monthlyInterestRate * remaining_balance)
    # print('remaining balance for month',month,'=',remaining_balance)
    month += 1
  if remaining_balance <= 0:
    # print('monthly payment', fixed_monthly)
  else:
    # print('nope- monthly', fixed_monthly)
    postBalance(balance, annualInterestRate, fixed_monthly + 10)

 


postBalance(4773, 0.2)